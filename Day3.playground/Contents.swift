...
func findClosestIntersection(firstPath: String, secondPath: String, using measurement: Measurememt) -> Int {
  // x,y position -> steps to get there from path1, steps path2
  var grid = [[Int]: [Int]]()

  let firstSplit = firstPath.split(separator: ",")
  let secondSplit = secondPath.split(separator: ",")

  updateGrid(&grid, path: firstSplit, isFirst: true)
  updateGrid(&grid, path: secondSplit, isFirst: false)

  // A path exists to this point from both first and second wires
  let intersections = grid.filter { $0.value.count == 2 }

  switch measurement {
  case .manhattan:
    var minDistance = Int.max
    for (key, _) in intersections {
      let currentDistance = abs(key[0]) + abs(key[1])
  ...
  case .steps:
    ...
    for (_, value) in intersections {
      let currentSteps = value[0] + value[1]
...
func updateGrid(_ grid: inout [[Int]: [Int]], path: [Substring], isFirst: Bool) {
  var startPoint = (0, 0)
  var steps = 1

  for instruction in path {
    let vector = getVector(String(instruction))
    switch vector.0 {
    case .right:
      let row = startPoint.0
      for col in stride(from: startPoint.1 + 1, through: startPoint.1 + vector.1, by: 1) {
        if isFirst {
          grid[[row, col]] = [steps]
        }
        else {
          if let arr = grid[[row, col]] {
            grid[[row, col]] = arr + [steps]
          }
        }
        steps += 1
      }
    ...
    case .up:
      ...
      for row in stride(from: startPoint.0 - 1, through: startPoint.0 - vector.1, by: -1) {
        if isFirst {
          grid[[row, col]] = [steps]
        }
        else {
...
func getVector(_ input: String) -> (Direction, Int) {
  let firstCharacter = input.first!
  let number = Int(String(input.dropFirst()))!

  if firstCharacter == "R" {
    return (Direction.right, number)
  } else if firstCharacter == "L" {
    return (Direction.left, number)
  } else if firstCharacter == "U" {
    return (Direction.up, number)
